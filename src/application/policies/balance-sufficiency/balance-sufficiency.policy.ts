...
@Injectable()
export class BalanceSufficiencyPolicy {
  constructor(
  ...
  async ensure(input: EnsureSufficientBalanceInput): Promise<void> {
    ...
    const tokenMaster = this.tokenDeploymentRepository.findTokenMasterByAddress({ chainKey, address: tokenAddress });
    if (!tokenMaster)
      throw new ApplicationError({ code: "TOKEN_ADDRESS_UNKNOWN", message: `Unknown token address: ${tokenAddress}` });

    const feeTokenMaster = this.tokenDeploymentRepository.findTokenMasterByAddress({
      chainKey,
